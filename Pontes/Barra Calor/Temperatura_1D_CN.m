...
k = (a*d)/(dx^2); %Constante da derivada parcial em x
...
for p=1:X
    ...
    elseif p == X
        ...
        X1(p,p-1) = -0.25*k; X1(p,p+1) = -0.25*k;
        X1(p,p) = (1+0.5*k);
        X2(p,p-1) = 0.25*k; X2(p,p+1) = 0.25*k;
        X2(p,p) = -0.5*k;
    end
end
...
for n=2:tmax
    for i=2:(X-1)
		...
		while ((abs(T(i,1,n)-T_ant(i,1)))/abs(T_ant(i,1))) > 0.005 %Enquanto diferenca entre T e T_ant for maior que 0,5%, a iteracao continua rodando
		    ...
		    %Cranck Nicholson
		    E1(i,1) = 0.25*k*[T(i-1,1,n-1)-2*T(i,1,n-1)+T(i+1,1,n-1)+T_ant(i-1,1)-2*T_ant(i,1)+T_ant(i+1,1)]+T(i,1,n-1) + Q(i,1);
